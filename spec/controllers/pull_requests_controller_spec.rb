    ...
    shared_examples "common behavior" do |git_server_type|
      ...
      context "for push events" do
        ...
        it "it should not error if the repository url in the request is not found" do
          repository_overrides = {"url" => "git@git.example.com:doesnot/exist.git", "host" => nil, "key" => nil, "slug" => nil}

          expect {
            post :build, 'payload' => push_payload("repository" => repository_overrides)
            expect(response).to be_success
          }.to_not change(Build, :count)
        end
      end
      ...
      context "for pull requests" do
        ...
        context "when the pull request sha has already been built" do
          ...
          it "still creates a build if the sha is used by a different repo" do
            ...
            repo2 = FactoryGirl.create(:repository, url: "git@git.example.com:square/other-repo.git")
  ...
  def stash_push_payload(options = {})
    {
      ...
      "repository" => {
        "id" => "252",
        "url" => "https://git.example.com/projects/SQUARE/repos/web/browse",
        "key" => "SQUARE",
        "slug" => "web",
        "name" => "web",
      },
