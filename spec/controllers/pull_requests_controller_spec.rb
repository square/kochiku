...
describe PullRequestsController do
    ...
    describe "post /pull-request-builder" do
      context "from github" do
        context "for push events" do
          ...
          it "creates the default project if required" do
            ...
            expect(Project.where(name: "web")).to_not exist
            expect {
              post :build, 'payload' => push_payload
              expect(response).to be_success
            }.to change(Project, :count).by(1)
            expect(Project.where(name: "web", repository: repository)).to exist
          end
          ...
          it "creates a build" do
            expect(Build.where(branch: "master", ref: to_40('2'))).to_not exist
            expect {
              post :build, 'payload' => push_payload
              expect(response).to be_success
            }.to change(Build, :count).by(1)
            expect(Build.where(branch: "master", ref: to_40('2'))).to exist
          end
        ...
        context "for pull requests" do
          ...
          it "creates the pull request project" do
            expect(Project.where(repository: repository, name: "web-pull_requests")).to_not exist
            expect {
              post :build, 'payload' => pull_request_payload
              expect(response).to be_success
            }.to change(Project, :count).by(1)
            expect(Project.where(repository: repository, name: "web-pull_requests")).to exist
          end

          it "creates a build for a pull request" do
            expect(Build.where(branch: "branch-name", ref: to_40('1'))).to_not exist
            expect {
              post :build, 'payload' => pull_request_payload
              expect(response).to be_success
            }.to change(Build, :count).by(1)
            expect(Build.where(branch: "branch-name", ref: to_40('1'))).to exist
          end
      ...
      context "from stash" do
        context "for push events" do
          ...
          it "creates the default project if required" do
            project.update_attributes!(:name => "web-something")
            expect(Project.where(repository: repository, name: "web")).to_not exist
            expect {
              post :build, 'payload' => push_payload_for_stash_plugin
              expect(response).to be_success
            }.to change(Project, :count).by(1)
            expect(Project.where(repository: repository, name: "web")).to exist
          end
          ...
          it "does not a build for not master" do
            expect {
              post :build, 'payload' => push_payload_for_stash_plugin("ref" => "refs/heads/some-branch")
              expect(response).to be_success
            }.to_not change(Build, :count)
          end
          ...
          it "it should not error if the repository url in the request is not found" do
            expect {
              post :build, 'payload' => push_payload_for_stash_plugin("repository" => {
                "key" => "square",
                "slug" => "doesnotexist",
              })
              expect(response).to be_success
        ...
        context "for pull requests" do
          ...
          it "creates the pull request project" do
            expect(Project.where(repository: repository, name: "web-pull_requests")).to_not exist
            expect {
              post :build, 'payload' => pull_request_payload_for_stash_plugin
              expect(response).to be_success
            }.to change(Project, :count).by(1)
            expect(Project.where(repository: repository, name: "web-pull_requests")).to exist
          end
          ...
          it "will enqueue a build if autobuild pull requests is enabled" do
            github_payload = pull_request_payload_for_stash_plugin("pull_request" => {
              "head" => {"sha" => to_40('1'), "ref" => "branch-name"},
              "body" => "best pull request ever",
            })
            expect {
          ...
          context "with a project" do
            ...
            it "does not create a pull request if not requested" do
              repository.update_attribute(:build_pull_requests, false)
              expect {
                post :build, 'payload' => pull_request_payload_for_stash_plugin({"pull_request" => {"body" => "don't build it"}})
                expect(response).to be_success
              }.to_not change(project.builds, :count)
            end
            ...
            it "does not build a closed pull request" do
              expect {
                post :build, 'payload' => pull_request_payload_for_stash_plugin({"action" => "closed"})
                expect(response).to be_success
              }.to_not change(project.builds, :count)
            end

            it "does not blow up if action is missing" do
              post :build, 'payload' => pull_request_payload_for_stash_plugin({"action" => nil})
              expect(response).to be_success
            end
        ...
        it "does not blow up if pull_request is missing" do
          expect {
            post :build, 'payload' => pull_request_payload_for_stash_plugin({"pull_request" => nil})
            expect(response).to be_success
          }.to_not change(Build, :count)
        end
        ...
        it "it should not error if the repository url in the request is not found" do
          expect {
            pr_payload = pull_request_payload_for_stash_plugin("repository" => {
              "key" => "square",
              "slug" => "doesnotexist",
            })
            post :build, 'payload' => pr_payload
  ...
  def push_payload_for_stash_plugin(options = {})
    {
      "after" => to_40('2'),
      "repository" => {
        "key" => "square",
  ...
  def pull_request_payload_for_stash_plugin(options = {})
    {
      "pull_request" => {
        "head" => {
          "sha" => to_40('1'),
